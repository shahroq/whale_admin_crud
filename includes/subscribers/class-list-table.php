...
class Subscribers_List_Table extends \WP_List_Table {
    ...
    public static function get_items( $per_page = 5, $page_number = 1 ) {
    	...
    	if ( ! empty( $_REQUEST['orderby'] ) ) {
        	...
        	$sql .= ' ORDER BY subscriber_id DESC ';
        }
    ...
    function column_email( $item ) {
    	...
    	//Build row actions
        $actions = array(
        	'edit'      => sprintf('<a href="?page=%s&action=%s&id=%s">Edit</a>',$_REQUEST['page'],'edit',$item['subscriber_id']),
         	'delete'    => sprintf('<a href="?page=%s&action=%s&id=%s">Delete</a>',$_REQUEST['page'],'delete',$item['subscriber_id']),
        );
    ...
    function column_cb( $item ) {
    	return sprintf(
        	'<input type="checkbox" name="bulk-delete[]" value="%s" />', $item['subscriber_id']
      	);
    }
